...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class ScreenSpaceReflections : PostProcessEffectSettings
    {
        ...
        /// <summary>
        /// smooth threshold for performance
        /// </summary>
        [Range(0f, 1f), Tooltip("Smooth threshold for cut ray tracing.")]
        public FloatParameter smoothThreshold = new FloatParameter { value = 0f };
    ...
    {
        /// <summary>
        /// forward gbuffer plugin
        /// </summary>
        [DllImport("ForwardGBuffer")]
        static extern void CacheForwardGBuffer(IntPtr _color, IntPtr _normal, IntPtr _specular);

        [DllImport("ForwardGBuffer")]
        static extern IntPtr GetRenderEventFunc();

        [DllImport("ForwardGBuffer")]
        static extern void ReleaseNative();

        RenderTexture m_Resolve;
        RenderTexture m_History;
        RenderTexture m_Diffuse;
        RenderTexture m_Normal;
        RenderTexture m_Specular;
        int[] m_MipIDs;
        bool forwardGBufferInit = false;
        ...
        internal void CheckRT(ref RenderTexture rt, int width, int height, FilterMode filterMode, bool useMipMap, RenderTextureFormat format, RenderTextureReadWrite rw)
        {
            if (rt == null || !rt.IsCreated() || rt.width != width || rt.height != height)
            {
                if (rt != null)
                ...
                rt = new RenderTexture(width, height, 0, format, rw)
                {
                    filterMode = filterMode,
                    useMipMap = useMipMap,
        ...
        public override void Render(PostProcessRenderContext context)
        {
            ...
            // create forward gbuffer if need
            if (context.camera.actualRenderingPath == RenderingPath.Forward)
            {
                CheckRT(ref m_Diffuse, context.width, context.height, FilterMode.Point, true, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                CheckRT(ref m_Normal, context.width, context.height, FilterMode.Point, true, RenderTextureFormat.ARGB2101010, RenderTextureReadWrite.Linear);
                CheckRT(ref m_Specular, context.width, context.height, FilterMode.Point, true, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                m_Diffuse.name = "Forward Diffuse";
                m_Normal.name = "Forward Normal";
                m_Specular.name = "Forward Specular";

                if (!forwardGBufferInit)
                {
                    CacheForwardGBuffer(m_Diffuse.GetNativeTexturePtr(), m_Normal.GetNativeTexturePtr(), m_Specular.GetNativeTexturePtr());
                    forwardGBufferInit = true;
                }
            }
            ...
            sheet.properties.SetFloat(ShaderIDs.SmoothThreshold, settings.smoothThreshold);
        ...
        public override void Release()
        {
            ...
            RuntimeUtilities.Destroy(m_Diffuse);
            RuntimeUtilities.Destroy(m_Normal);
            RuntimeUtilities.Destroy(m_Specular);
            m_Resolve = null;
            m_History = null;
            m_Normal = null;
            m_Specular = null;
        ...
        public override void SetupForwardGBuffer(CommandBuffer cmd)
        {
            // hack Unity's render target
            cmd.IssuePluginEvent(GetRenderEventFunc(), 0);

            // setup gbuffer for use
            if (forwardGBufferInit)
            {
                cmd.SetGlobalTexture("_CameraGBufferTexture0", m_Diffuse);
                cmd.SetGlobalTexture("_CameraGBufferTexture1", m_Specular);
                cmd.SetGlobalTexture("_CameraGBufferTexture2", m_Normal);
            }
        }
    }
}
