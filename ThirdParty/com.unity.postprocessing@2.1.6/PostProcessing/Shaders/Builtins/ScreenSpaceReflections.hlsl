...
{
    float4 gbuffer2 = _CameraGBufferTexture2.Load(uint3(i.texcoordStereo * _ScreenParams.xy, 0));
	float smoothness = _CameraGBufferTexture1.Load(uint3(i.texcoordStereo * _ScreenParams.xy, 0)).a;

	[branch]
    if (dot(gbuffer2, 1.0) == 0.0 || smoothness < _SmoothThreshold)
        return 0.0;
